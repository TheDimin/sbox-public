...
public class SimsMount : BaseGameMount
{
	...
	protected override Task Mount( MountContext context )
	{
		...
		var files = System.IO.Directory.EnumerateFiles( dataDir, "*.package", SearchOption.AllDirectories );
	...
	{
		using ( var package = DbpfPackage.Open( file ) )
		{
			int count = 0;
			foreach ( var item in package.GetModelRecords() )
			{
				string DisplayName = "";
				...
				var mountPath = $"mount://{Ident}/Models/{DisplayName}";

				context.Add( Sandbox.Mounting.ResourceType.Model, mountPath, new ModelLoader( item ) );
				count++;
				if ( count > 5 )
					break;
			}
