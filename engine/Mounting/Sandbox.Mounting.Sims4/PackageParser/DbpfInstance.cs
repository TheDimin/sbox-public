...
public sealed class DbpfInstance : IDisposable
{
    ...
    {
        var stream = new FileStream(path, FileMode.Open, FileAccess.Read, FileShare.Read, bufferSize: 64 * 1024, FileOptions.RandomAccess | FileOptions.SequentialScan);

        Span<byte> headerBuffer = stackalloc byte[DbpfHeader.Size];
        ...
        {
            stream.Dispose();
            throw new InvalidDataException("Not a DBPF file (missing DBPF magic)");
        }
