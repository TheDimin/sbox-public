...
public sealed class GEOMResource
{
    ...
    public GEOMResource(byte[] data)
    {
        ...
        reader.Skip(checked((int)(publicKeyCount * StructUtil.SizeOf<ResourceKey>())));
        reader.Skip(checked((int)(externalKeyCount * StructUtil.SizeOf<ResourceKey>())));
        reader.Skip(checked((int)(delayLoadKeyCount * StructUtil.SizeOf<ResourceKey>())));

        ReadOnlySpan<byte> objectDataSpan = reader.ReadSpan(checked((int)(objectCount * StructUtil.SizeOf<ObjectData>())));
        ...
        _geomOffset = checked((int)objectData[0].Position);
        reader.Seek(_geomOffset);
